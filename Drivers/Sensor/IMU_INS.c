...
//¬ременныке переменные дл€ сборки
SemaphoreHandle_t xParcel_TX_Semaphore = NULL;
extern volatile float throtle_ppm;
extern SemaphoreHandle_t xMAG_Mutex;
extern SemaphoreHandle_t xBARO_Mutex;
...
uint32_t deltaTimeUKF,   executionTimeUKF ,   previousUKFTime;
extern uint32_t deltaTimeGPScalib;
float dtUKF;

extern TIM_HandleTypeDef htim7;
portBASE_TYPE xHigherPriorityTaskWoken_UKF = pdFALSE;

void TIM7_IRQHandler(void)
{
  /* USER CODE BEGIN TIM1_TRG_COM_TIM7_IRQn 0 */
 // LED_Toggle(3);
   xSemaphoreGiveFromISR(xUKF_Semaphore,  &xHigherPriorityTaskWoken_UKF);
  /* USER CODE END TIM1_TRG_COM_TIM7_IRQn 0 */
  HAL_TIM_IRQHandler(&htim7);
  /* USER CODE BEGIN TIM1_TRG_COM_TIM7_IRQn 1 */

  /* USER CODE END TIM1_TRG_COM_TIM7_IRQn 1 */
}
...
void UKF_Cycle(void)
{
	...
	else if (flag_start_state_init == 2)
	{
		...
		//	DeltaTimeUKF = (float)deltaTimeUKF * 0.0000001f;
			DeltaTimeUKF = dtUKF;
			...
			//taskENTER_CRITICAL();
			xSemaphoreTake(xBARO_Mutex, portMAX_DELAY);
			runData.presHist[runData.sensorHistIndex] = Altitude;
			xSemaphoreGive(xBARO_Mutex);
			//taskEXIT_CRITICAL();
			runData.sumPres += runData.presHist[runData.sensorHistIndex];
...
void prvIMU_INS_UKF(void *pvParameters)
{
	...
	while(1)
	{
	    ...
	    deltaTimeUKF    = abs(currentTime - previousUKFTime);
	    previousUKFTime = currentTime;
	    dtUKF = deltaTimeUKF * 0.0000001f;
//	    dtUKF = ((float)deltaTimeUKF/(float)deltaTimeGPScalib); // вариант с использованием калиброванного времени по GPS

	//	cnt_cycle_ukf = (uint32_t)abs(TIM5->CNT - cnt_ticks_ukf);
	//	cnt_ticks_ukf = (uint32_t)TIM5->CNT;

		UKF_Cycle();

	//	Control();   //перенесено из IMU_INS

	    executionTimeUKF = TIM5->CNT - previousUKFTime;
	}
...
//	xSemaphoreGive(xUKF_PID_Mutex);
	...
	q0z = cosf(ez_/2.0f);
	q3z = -sinf(ez_/2.0f);
	//дл€ оси У придетс€ повозитьс€, убираем вращение по оси Z из глобального кватерниона
...
void Fast_mahony(float *acc, float *gyr, float *yVec )
	...
	if(!((acc[0] == 0.0f) && (acc[1] == 0.0f) && (acc[2] == 0.0f)))
		...
		gyr[0] *= (0.5f*dtINS);		// pre-multiply common factors
		gyr[1] *= (0.5f*dtINS);
		gyr[2] *= (0.5f*dtINS);
		qa = qmah0;
		qb = qmah1;
